(function(){
  ...
  document.addEventListener('DOMContentLoaded',async function(){
    try{
      var all=await fetch('/content/all.json'+bust).then(r=>r.json());
      var d=all.design||{}, s=all.site||{}, h=all.home||{}, sv=all.services||{}, g=all.gallery||{}, c=all.contact||{}, cu=all.customers||{};
      ...
      /* SITE */
      updateLinks(s.phone,s.email);
      setAll('.footer-tagline',s.footer_tagline);
      ...
      if(document.querySelector('.hero')){
        ...
        if(sl[1]&&h.printing_materials)sl[1].innerHTML=h.printing_materials.map(function(x){return'<li>'+x+'</li>';}).join('');
      ...
      if(document.querySelector('.service-detail')){
        ...
        if(sv.services)sv.services.forEach(function(svc){
          ...
          if(sp&&svc.specs)sp.innerHTML=svc.specs.map(function(x){return'<div class="service-spec"><span class="service-spec-label">'+x.spec_label+'</span><span class="service-spec-value">'+x.value+'</span></div>';}).join('');
        });
